...
def draw_receipt_icon(draw, cx, cy, size):
    """Small receipt-style icon: a rounded rect with a torn/zigzag bottom
    edge and a $ glyph inside -- approximates lucide's Receipt icon
    (real icon used on this button live) at marketing-asset scale rather
    than reproducing its exact SVG path."""
    half = size / 2
    left, top, right, bottom = cx - half, cy - half, cx + half, cy - half * 0.35
    draw.rounded_rectangle((left, top, right, bottom), radius=2, outline=NAVY, width=2)
    font = ImageFont.truetype(FONT_PATH, int(size * 0.62))
    label = "$"
    bbox = draw.textbbox((0, 0), label, font=font)
    tw, th = bbox[2] - bbox[0], bbox[3] - bbox[1]
    icon_cy = (top + bottom) / 2
    draw.text((cx - tw / 2 - bbox[0], icon_cy - th / 2 - bbox[1]), label, font=font, fill=NAVY)
