...
-- (src/lib/summarize-document.ts). Defaults to Anthropic (the only
-- provider until now) — Mistral is an opt-in alternative for anyone who'd
-- rather their document text not go to Anthropic specifically, using a
-- single server-side Mistral API key (MISTRAL_API_KEY env var), not a
-- per-org bring-your-own-key setup.
--
