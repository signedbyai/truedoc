"use client";
// Same client-component requirement as homepage-es-ar.tsx (TABS_ES embeds
// lucide-react icon COMPONENTS passed as a prop into the client-only
// InteractiveProductTabsG) — see that file's top comment for the full
// "why `use client`" explanation, not repeated here.
...
// Peninsular Spanish (Spain, tuteo) homepage variant — built 2026-08-22,
// direct ask, once the Argentina Reddit campaign's audience turned out to
// be small (~50-60k) and DE/FR were confirmed already live. This is
// homepage-es-ar.tsx with every voseo verb form (empezá, enviá, conectá,
// etc.) converted to standard tuteo (empieza, envía, conecta) for a Spain
// audience — vos-form Spanish reads as distinctly Argentine/Rioplatense to
// a Spain reader, the same way "y'all" would read as regional to a UK
// reader, so a straight reuse of the AR copy would have undercut the
// dialect-matching this whole DE/FR/ES-ES push is for. Same
// [[site-localization-scope-2026-08-20]] scope, same
// fast/cheap/reversible-test framing as homepage-es-ar.tsx's own comment.
//
// Same accepted limitation as the AR variant: product screenshots still show
// the English app UI (full dashboard translation is out of scope for this
// pass). "Sin vueltas" is kept as the tagline here too — it's standard
// Spanish, not vos-marked, and reads naturally in Spain as well as
// Argentina (it doesn't have the same regional-tell problem as the verb
// conjugations elsewhere on this page).
...
const FAQS = [
  ...
  },
  {
    ...
    a: "Sí. Con Seal puedes autofirmar y sellar un documento con un sello con marca de tiempo confiable RFC 3161 e identidad verificada — sin necesidad de destinatario.",
  },
  {
...
const VALUE_PROPS: { label: string; path: string }[] = [
  { label: "Envía más rápido", path: "M22 2 11 13M22 2l-7 20-4-9-9-4 20-7z" },
  { label: "Sigue el progreso", path: "M3 12h4l3 8 4-16 3 8h4" },
  { label: "Controla el acceso", path: "M5 11h14v10H5zM8 11V7a4 4 0 0 1 8 0v4" },
  { label: "Cierra acuerdos", path: "M12 3a9 9 0 1 0 0 18 9 9 0 0 0 0-18zM9 12l2 2 4-4" },
];
...
const TABS_ES: ProductTab[] = [
  {
    key: "sign",
    title: "Firmar",
    description: "Agrega campos de firma, iniciales, fecha y texto en cualquier PDF, y envíalo para firmar en segundos.",
  ...
  },
  {
    ...
    title: "Sellar",
    description:
      "Autofirma y sella un documento con un sello con marca de tiempo confiable RFC 3161 e identidad verificada — sin necesidad de destinatario.",
  ...
  },
  {
    key: "quote",
    title: "Presupuesto",
    description: "Describe el trabajo en lenguaje simple y Magic Quote lo convierte en un presupuesto detallado, listo para firmar.",
  ...
  },
  {
    key: "draft",
    title: "Redactar",
    description: "Describe lo que necesitas y la IA redacta un acuerdo listo para enviar — revísalo, edítalo y envíalo en el mismo paso.",
...
export function HomepageEsEs({ currency }: { currency: Currency }) {
  return (
    <>
      ...
      <section className="mx-auto grid w-full max-w-6xl gap-10 px-6 pt-12 pb-16 sm:grid-cols-2 sm:items-start sm:pt-16">
        <div className="min-w-0 text-center sm:text-left">
          ...
          >
            <span className="hidden lg:inline">Los equipos ahorran</span>
            <span className="lg:hidden">Ahorra</span>
            <span className="font-bold text-yellow-300">$700+/año</span>
            <span>vs DocuSign</span>
            <span className="hidden lg:inline">— mira los números</span>
          ...
          <div className="mt-9 flex flex-col items-center sm:items-start">
            <CtaLink
              href="/login?intent=signup&utm_source=homepage_es_es&utm_medium=cta&utm_campaign=homepage_page_es_es&utm_content=preview-g-es-es"
              color="purple"
              page="homepage-es-es"
              position="hero"
              variant="es-es"
            ...
            <p className="mt-3 text-xs text-slate-400">No necesitas tarjeta de crédito — 3 documentos gratis por mes.</p>
          </div>
        </div>
        ...
        <div className="grid gap-10 sm:grid-cols-2 sm:items-center">
          <div className="text-center sm:text-left">
            <div className="mb-3 flex items-center justify-center gap-2.5 sm:justify-start">
              ...
              />
              <h2 className="text-lg font-semibold text-slate-900">Conecta tu CRM</h2>
            </div>
            <p className="mx-auto max-w-sm text-slate-600 sm:mx-0 sm:max-w-none">
              Una API REST y webhooks salientes — crea y envía documentos desde tu CRM, consulta el estado o
              recibe una notificación en cuanto se firme algo.
            </p>
            <Link
              href="/developers"
              className="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-slate-900 hover:text-slate-700"
            >
              Mira la documentación de la API <span aria-hidden>→</span>
            </Link>
          </div>
