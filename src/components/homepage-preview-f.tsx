...
import { InteractiveProductTabsF } from "@/components/homepage-preview-f-tabs";

// Variant F, direct ask 2026-08-13: "sure lets an alternative to
// compare" — identical to E in every respect except the mobile tab-pill
// treatment. E answered "4 pills don't fit one row on an iPhone" with a
// deliberate 2x2 grid; F is the other real option, icon-only pills in a
// single row on mobile (see homepage-preview-f-tabs.tsx's own comment).
// Everything below the hero — DocuSign comparison, dev panel, trusted-by,
// pricing, FAQ — is an unmodified copy of homepage-preview-e.tsx. Keep
// both in sync if the shared sections change; this file exists purely so
// the two pill treatments can sit side by side on live, separate URLs.
...
export function HomepagePreviewF({ currency }: { currency: Currency }) {
  return (
    <>
      ...
      <section className="mx-auto grid w-full max-w-6xl gap-10 px-6 pt-12 pb-16 sm:grid-cols-2 sm:items-start sm:pt-16">
        <div className="text-center sm:text-left">
          ...
          <div className="mt-9 flex flex-col items-center sm:items-start">
            <CtaLink
              href="/login?intent=signup&utm_source=homepage&utm_medium=cta&utm_campaign=homepage_page&utm_content=preview-f"
              color="purple"
              page="homepage"
              position="hero"
              variant="preview-f"
