...
// Dashboard referral: "give a month, get a month". On mount it (1) claims any
// ?ref code stashed by ReferralCapture, and (2) loads this org's own referral
// link + conversions. Now a compact tappable card (gift + yellow sparkles) that
// pops open a share sheet — link + native share button — instead of the old
// inline link box. Dismissible; the claim runs regardless of whether it shows.
export function ReferralCard() {
  ...
  async function share() {
    ...
    const data = {
      title: "SignedBy",
      text: "Give a month, get a month on SignedBy — sign up with my link:",
      url: link,
    };
      ...
      <div className="relative flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 py-3 pl-3 pr-9">
        <button type="button" onClick={() => setOpen(true)} className="flex flex-1 items-center gap-3 text-left">
          ...
          <span className="min-w-0">
            <span className="block text-sm font-semibold text-slate-900">Give a month, get a month</span>
