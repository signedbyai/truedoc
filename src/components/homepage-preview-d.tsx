...
import { InteractiveProductTabs } from "@/components/homepage-preview-d-tabs";

// Variant D, direct ask 2026-08-12: "freedom to make your own." Genuinely
// its own structure, not a remix of B/v26/C — differs from all three in
// three real ways, not just visual polish:
// 1. Two-column hero (headline+CTA left, an interactive Sign/Seal/Quote/
//    Draft tab switcher right — see homepage-preview-d-tabs.tsx) instead
//    of every other variant's single centered column. The tabs are a real
//    design decision, not decoration: a visitor clicking between them is a
//    stronger engagement signal than watching an auto-playing loop, and it
//    sidesteps the whole class of crossfade timing bugs this session spent
//    many passes fixing.
// 2. A real numeric DocuSign comparison inline on the homepage itself
//    (pulled from the actual published figures on /vs/docusign, not
//    invented), rather than only a pill that links out to it.
// 3. A short FAQ, grounded ONLY in facts already stated elsewhere on the
//    site (free-plan terms, EEA data residency, the AI-training claim,
//    Seal's no-recipient-required self-signing) — no new claims invented
//    for this page.
// Drops the FEATURES card grid (C already covers that combination) and the
// Developer/API panel's neighbors are kept since that content has no
// equivalent elsewhere in this variant.
//
// Real DocuSign figures below, copied verbatim from
// src/app/vs/docusign/page.tsx (COMPARISON_ROWS) rather than restated from
// memory — keep the two in sync if either page's pricing changes.
...
export function HomepagePreviewD({ currency }: { currency: Currency }) {
  return (
    <>
      ...
      {/* Two-column hero — the one structural break from every other
          variant, all of which center-stack text above/below a single
          image. */}
      <section className="mx-auto grid w-full max-w-6xl gap-10 px-6 pt-12 pb-16 sm:grid-cols-2 sm:items-center sm:pt-16">
        <div className="text-center sm:text-left">
          ...
          <p className="mx-auto mt-4 max-w-md text-lg text-slate-600 sm:mx-0">
          ...
          <div className="mt-7 flex flex-col items-center sm:items-start">
            <CtaLink
              href="/login?intent=signup&utm_source=homepage&utm_medium=cta&utm_campaign=homepage_page&utm_content=preview-d"
              color="purple"
              page="homepage"
              position="hero"
              variant="preview-d"
      ...
      {/* Developer/API panel — same real content as B/C, no equivalent
      ...
      {/* FAQ — new to this variant, grounded only in facts already stated
          elsewhere on the site (see this file's own top comment). */}
