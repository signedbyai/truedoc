...
import { ContractEndDateCard } from "@/components/contract-end-date-card";
...
export default async function DocumentEditorPage({
  ...
  const { data: doc } = await supabase
    .from("documents")
    .select(
      "id, org_id, title, page_count, status, signed_file_path, is_verified_badge, certificate_file_path, certificate_mode, badge_stamped_file_path, payment_link_url, payment_label, docgate_url, docgate_label, open_notifications, recipient_notice, invite_subject, invite_message, expires_at, contract_end_date, organizations(plan, auto_suggest_on_upload)"
    )
    .eq("id", id)
    .single();
    ...
    return (
      <main className="px-4 py-8 sm:px-6 sm:py-10">
        <div className="mx-auto max-w-2xl space-y-6">
          ...
          {/* Contract end-date reminder (CONTRACT_END_DATE_REMINDER_
              SCOPE.md, lawyer discovery-call feedback) -- the underlying
              contract's own term-end date, distinct from expires_at. Free
              on every plan, any tier. Placed first among the cards below
              since it's a document-level property a lawyer wants to see
              immediately on a completed contract. */}
          <ContractEndDateCard documentId={id} initialContractEndDate={doc.contract_end_date} />
